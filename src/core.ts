...
export interface PCEvent {
	...
	joinUrl?: string;
	recurring?: boolean;
	/** The original occurrence time behind a recurring instance, before any
	 *  override moved it. Local ICS uses it to update the same exception. */
	recurrenceIdMs?: number;
	/** Whether that original occurrence was an all-day value. */
...
export type ViewMode = "month" | "week" | "workweek" | "day" | "agenda" | "tasks";
...
/* ---------- Microsoft account kind ---------- */

/** The tenant every personal Microsoft account signs in under. The id_token's
 *  tid claim carries it, which is how an account's kind is known for certain
 *  rather than inferred from its address. */
export const MSA_TENANT = "9188040d-6c67-4c5b-b112-36a304b66dad";

/** The addresses Microsoft hands out for personal accounts. Only a fallback:
 *  a personal account can carry a custom domain, so this decides nothing that
 *  the tid claim has already answered. */
const MSA_DOMAINS = new Set(["outlook.com", "hotmail.com", "live.com", "msn.com", "passport.com"]);

/** Whether an account is a personal Microsoft account rather than a work or
 *  school one, which decides what it can be asked to consent to.
 *
 *  `personal` is the tid claim's verdict from the last sign-in and settles it
 *  outright. Without one, meaning a row connected before that was recorded or
 *  an account about to sign in for the first time, the authority answers next,
 *  since 'consumers' admits personal accounts only, and the address answers
 *  last. `tenant` is the resolved authority, not the raw stored field. */
export function isPersonalMicrosoft(personal: boolean | undefined, tenant: string, label: string): boolean {
	if (personal !== undefined) return personal;
	if (tenant.trim().toLowerCase() === "consumers") return true;
	return MSA_DOMAINS.has((label.split("@")[1] ?? "").trim().toLowerCase());
}
...
/** Categories in the order the user arranged them, the same idiom the folder
 *  tree uses. A name the user has dragged into place leads, in that order, and
 *  everything untouched keeps the mailbox's own order behind it, so arranging
 *  two categories does not scramble the other twenty and a category made later
 *  still appears rather than vanishing. Matching is on the display name, which
 *  is the only stable handle a category has (Graph refuses to rename one), and
 *  case-insensitive to agree with how names are compared everywhere else. */
export function orderCategories<T extends { displayName: string }>(cats: T[], custom?: string[]): T[] {
	if (!custom?.length) return cats;
	const rank = new Map(custom.map((n, i) => [n.toLowerCase(), i]));
	return cats
		.map((c, i) => ({ c, i })) // the mailbox's index is the tiebreaker, so the sort stays stable
		.sort((a, b) => {
			const ra = rank.get(a.c.displayName.toLowerCase());
			const rb = rank.get(b.c.displayName.toLowerCase());
...
export interface EventDraft {
	...
	/** Attach the provider's own online meeting and let it mint the join link:
	 *  a Teams meeting on Microsoft 365, a Google Meet on Google. Create only. */
...
export function graphEventBody(d: EventDraft, tz: string): Record<string, unknown> {
	...
	// ask Exchange to host it on Teams: it mints the join link and threads it
	// into the body itself, exactly as Outlook's own "Teams meeting" toggle does
	if (d.onlineMeeting) {
...
export function googleEventBody(d: EventDraft, tz: string): Record<string, unknown> {
	...
	// Google mints a Meet link when it sees a createRequest. The request id must
	// be fresh each time: reuse a previous one and Google quietly ignores the
	// ask and returns no conference, so a second identical event would land
	// without a link. The write must also carry conferenceDataVersion=1, which
	// insertGoogleEvent adds when it sees this field.
	if (d.onlineMeeting) {
