...
/** One local .ics calendar living in the vault. */
interface LocalCalendarSource {
	id: string;
	name: string;
	/** Vault-relative path, e.g. "Calendar/My Calendar.ics". */
	path: string;
	color: string;
	enabled: boolean;
}
...
interface GraphAccount {
	...
	mail?: boolean;
	/** A personal Microsoft account, which can be asked for less than a work
	 *  one (see GRAPH_PERSONAL_SCOPE). Set from the id_token's tid claim at
	 *  sign-in, so it is known rather than guessed; undefined on rows that
	 *  last signed in before this was recorded, which fall back to the
	 *  address. */
	personal?: boolean;
}
...
interface NyaHomeSettings {
	...
	/** Where the mail view opens. "all" is the unified list, "last" is wherever
	 *  you were when you closed it, and "folder" is one you picked and pinned
	 *  with "Open here when mail opens". The folder fields carry that pick. */
	mailStart: { mode: "all" | "last" | "folder"; accountId: string; folderId: string; name: string };
	/** The folder the view was last looking at, kept for the "last" mode.
	 *  Null means the unified list, which is also what "all" selects. */
	mailLastFolder: { accountId: string; folderId: string; name: string } | null;
	/** Drop the All inboxes row from the top of the pane. The palette still
	 *  reaches the unified list, so hiding it never strands anyone. */
	mailHideAllInboxes: boolean;
	...
	/** Categories dragged into an order of their own, per account, by name.
	 *  Same idiom as mailFolderOrder: the named ones lead, the rest keep the
	 *  mailbox's own order behind them, so a category made later still shows. */
	mailCategoryOrder: { accountId: string; names: string[] }[];
...
/** An explicitly saved IMAP draft. Attachments are stored as base64 because
 *  a draft must survive an Obsidian restart; large files still belong in a
 *  real folder, but ordinary documents are fine in data.json. */
interface ImapDraft {
	id: string;
	accountId: string;
	to: string;
...
const DEFAULT_SETTINGS: NyaHomeSettings = {
	...
	mailStart: { mode: "all", accountId: "", folderId: "", name: "" },
	mailLastFolder: null,
	mailHideAllInboxes: false,
	mailCollapsed: [],
	markRead: "select",
	markReadSeconds: 5,
...
type SourceDef =
	...
	| { kind: "local"; key: string; label: string; source: LocalCalendarSource; color: string }
...
export default class NyaHomePlugin extends Plugin {
	...
	async onload() {
		...
		if (stale.length) {
			...
			if (!Platform.isMobileApp || this.app.loadLocalStorage(key) !== "1") {
				new Notice(message, 10000);
				if (Platform.isMobileApp) this.app.saveLocalStorage(key, "1");
			}
		}
		...
		this.app.workspace.onLayoutReady(() => {
			if (Platform.isMobileApp) {
				const t = window.setTimeout(() => this.checkReminders(), 15_000);
				this.register(() => window.clearTimeout(t));
			} else this.checkReminders();
		});
		...
		this.app.workspace.onLayoutReady(() => {
			if (Platform.isMobileApp) {
				const t = window.setTimeout(() => void this.wakeSnoozes(), 30_000);
				this.register(() => window.clearTimeout(t));
			} else void this.wakeSnoozes();
		});
		...
		// Obsidian restores the workspace before plugins settle, so this runs
		// once the leaves exist. A manual enable mid-session is already ready,
	...
	private checkReminders() {
		...
		if (Platform.isMobileApp && due.length > 1) {
			for (const ev of due) this.remindersFired.add(ev.id);
			this.showReminderBatch(due);
			return;
		}
		for (const ev of due) {
	 ...
	 *  overlapping events. Desktop keeps one full card per event, where the
	 *  notice rail has room for them. */
	private showReminderBatch(events: PCEvent[]) {
		let notice: Notice | null = null;
	...
	sources(): SourceDef[] {
		...
		for (const lc of this.settings.localCalendars) {
			if (!lc.enabled) continue;
			out.push({ kind: "local", key: `local:${lc.id}`, label: lc.name, source: lc, color: lc.color || paletteColor(i) });
			i++;
		}
	...
	notify() {
		...
		// Calendar and mail fetches notify at start, per account/source, and at
		// finish. A vault sync adds another dense stream. Keep the cached paint in
		// place and deliver one trailing redraw when the phone is settled.
		if (this.notifyTimer != null) return;
		this.notifyTimer = window.setTimeout(() => {
			this.notifyTimer = null;
			if (this.mobileStartupSettling()) {
				this.notify();
				return;
			}
			for (const l of Array.from(this.listeners)) l();
	...
	private mobileStartupSettling(): boolean {
		const pc = (
	...
	private async fetchSource(def: SourceDef, fromMs: number, toMs: number) {
		...
		try {
			...
			if (def.kind === "m365") {
				...
				const text = await this.readIcsText(path);
				// a missing or empty file is a fresh calendar, not an error
				events = text.trim()
					? parseIcsEvents(text, fromMs, toMs, { sourceId: def.key, calendarName: def.label, color: def.color }).map((e) => ({
						...e,
						canEdit: true,
						// the VEVENT UID behind "uid:startMs"; without it an edit
						// cannot find its row in the file and would append instead
						seriesId: e.id.slice(0, e.id.lastIndexOf(":")),
					}))
					: [];
			} else {
	...
	isPersonalAccount(a: GraphAccount): boolean {
		return isPersonalMicrosoft(a.personal, this.tenantFor(a), a.label);
	}

	/** The scope to ask this account for. Asking a personal account for a
	 *  work-only permission does not degrade, it hangs the sign-in. */
	scopeFor(a: GraphAccount): string {
		return this.isPersonalAccount(a) ? GRAPH_PERSONAL_SCOPE : GRAPH_SCOPE;
	}
	...
	private async graphTokenFor(a: GraphAccount): Promise<string> {
		...
		let t;
		try {
			t = await refreshTokens(this.clientIdFor(a), this.tenantFor(a), a.refresh, a.grantedScope || this.scopeFor(a));
		} catch (e) {
	...
	async deviceCodeSession(a: GraphAccount, isNew: boolean, onCode: (dc: DeviceCode) => void, cancelled: () => boolean, personalHint?: boolean): Promise<GraphAccount | null> {
		// the scope has to be chosen before anyone signs in, so a brand-new
		// account takes the wizard's word for which kind it is; the tid claim
		// below replaces the guess with the answer once the tokens arrive
		if (personalHint !== undefined && a.personal === undefined) a.personal = personalHint;
		const scope = this.scopeFor(a);
		const dc = await startDeviceCode(this.clientIdFor(a), this.tenantFor(a), scope);
		onCode(dc);
		...
		while (Date.now() < deadline && !cancelled()) {
			...
			if (who) a.label = who;
			// the tid claim names the directory that signed this in, so it
			// tells work from personal outright; a token without one leaves
			// whatever was already known standing rather than overwriting it
			const tid = claims?.tid as string | undefined;
			if (tid) a.personal = tid.toLowerCase() === MSA_TENANT;
			...
			if (target !== a) {
				...
				target.personal = a.personal;
			}
			target.access = res.access_token;
			target.refresh = res.refresh_token;
			target.expiry = Date.now() + res.expires_in * 1000;
			// what was actually asked for, so the "reconnect to enable" nudge
			// does not keep pointing at a permission this account cannot hold
			target.grantedScope = scope;
	...
	private refetchRemote(onlyKey?: string) {
		for (const def of this.sources()) {
			if (def.kind !== "m365" && def.kind !== "google" && def.kind !== "local") continue;
	...
	/* ---------------- local calendar writes ---------------- */

	/** Create or update one event in the source's .ics file. `ev` null means
	 *  create with a fresh UID; otherwise the event's seriesId (its VEVENT
	 *  UID in the file) is reused so the same row is edited in place. */
	private async writeLocalEvent(def: Extract<SourceDef, { kind: "local" }>, ev: PCEvent | null, draft: EventDraft): Promise<boolean> {
		const path = normalizePath(def.source.path);
		try {
			const dir = path.includes("/") ? path.slice(0, path.lastIndexOf("/")) : "";
			if (dir) await this.ensureFolder(dir);
			// seriesId can be missing on events restored from the persisted
			// cache (saved before it was set); the UID is recoverable from the
			// "uid:startMs" id, so the write always finds its own row
			const uid = ev?.seriesId || (ev ? ev.id.slice(0, ev.id.lastIndexOf(":")) : "") || newIcsUid();
			...
			if (ev?.recurring) {
				...
				// override (RECURRENCE-ID), the way Outlook splits one instance
				const recurrenceIdMs = ev.recurrenceIdMs ?? ev.startMs;
				text = upsertIcsOverride(text, uid, recurrenceIdMs, rec, ev.recurrenceIdAllDay ?? ev.allDay);
			} else {
				const repeat = ev ? null : rruleOf(draft.repeat ?? "none", draft.startMs);
				text = upsertIcsEvent(text, { ...rec, repeat: repeat ?? undefined });
			}
	...
	/** Delete one event's VEVENT row from the .ics file. */

	/** The calendar file's text, or "" when it does not exist yet. An earlier
	 *  build mis-created a FOLDER named like the file; reading that died with
	 *  EISDIR, so a directory at the file path is cleared on sight. */
	private async readIcsText(path: string): Promise<string> {
		if (!(await this.app.vault.adapter.exists(path))) return "";
		const st = await this.app.vault.adapter.stat(path);
		if (st?.type === "folder") {
			// an earlier build mis-created a folder where the file belongs.
			// Node refuses a non-recursive rm on a directory (EISDIR), and a
			// recursive rm would be unthinkable if someone kept real files
			// in there: only an EMPTY folder is cleared on sight.
			const inside = await this.app.vault.adapter.list(path);
			if (inside.files.length || inside.folders.length) {
				throw new Error("a folder named like the calendar file is in the way; move its contents elsewhere and delete the folder, then try again");
			}
	...
	private async removeLocalEvent(def: Extract<SourceDef, { kind: "local" }>, ev: PCEvent): Promise<void> {
		const path = normalizePath(def.source.path);
		const uid = ev.seriesId || ev.id.slice(0, ev.id.lastIndexOf(":"));
		...
		try {
			const old = await this.readIcsText(path);
			if (old) {
			const next = ev.recurring
				? removeIcsOccurrence(old, uid, ev.recurrenceIdMs ?? ev.startMs, ev.recurrenceIdAllDay ?? ev.allDay)
	...
	writableTargets(): { key: string; label: string; def: SourceDef }[] {
		...
		const locals: { key: string; label: string; def: SourceDef }[] = [];
		for (const def of this.sources()) {
			if (def.kind === "local") {
				locals.push({ key: def.key, label: def.label, def });
			} else if ((def.kind === "m365" || def.kind === "google") && def.writable) {
	...
	async moveEvent(ev: PCEvent, startMs: number, endMs: number): Promise<void> {
		...
		if (def?.kind === "local") {
			const ok = await this.writeLocalEvent(def, ev, {
	...
	async updateCalEvent(ev: PCEvent, draft: EventDraft): Promise<boolean> {
		const def = this.sourceByKey(ev.sourceId);
		if (def?.kind === "local") {
			const ok = await this.writeLocalEvent(def, ev, draft);
			if (ok) {
				const st = this.cache.get(ev.sourceId);
				const cached = st?.events.find((e) => e.id === ev.id);
				if (cached) {
					cached.title = draft.title.trim() || "(no title)";
					cached.startMs = draft.startMs;
					cached.endMs = draft.endMs;
					cached.allDay = draft.allDay;
					if (draft.location != null) cached.location = draft.location || undefined;
					if (draft.description != null) cached.description = draft.description || undefined;
					cached.transparent = draft.showAs === "free";
					cached.tentative = draft.showAs === "tentative";
				}
				this.notify();
				this.refetchRemote(def.key);
			}
	...
	async deleteCalEvent(ev: PCEvent): Promise<void> {
		const def = this.sourceByKey(ev.sourceId);
		if (def?.kind === "local") {
			await this.removeLocalEvent(def, ev);
			return;
		}
		if (!def || (def.kind !== "m365" && def.kind !== "google")) return;
		const st = this.cache.get(ev.sourceId);
	...
	categoryOrderFor(accountId: string): string[] {
		return this.settings.mailCategoryOrder.find((o) => o.accountId === accountId)?.names ?? [];
	}

	/** Put one category where another sits, by name. Unlike folders there is no
	 *  parent to respect: categories are a flat list, so the whole current order
	 *  is seeded and the dragged one spliced to the target's place, and that
	 *  full list becomes the account's order. */
	reorderCategory(accountId: string, dragName: string, targetName: string) {
		if (dragName === targetName) return;
		const names = this.categoriesFor(accountId).map((c) => c.displayName);
		const from = names.findIndex((n) => n.toLowerCase() === dragName.toLowerCase());
		const to = names.findIndex((n) => n.toLowerCase() === targetName.toLowerCase());
		if (from < 0 || to < 0) return;
		names.splice(to, 0, names.splice(from, 1)[0]);
		this.settings.mailCategoryOrder = [...this.settings.mailCategoryOrder.filter((o) => o.accountId !== accountId), { accountId, names }];
		this.queueSave();
		this.notify();
	}

	/** Give one account's categories back the mailbox's own order. */
	resetCategoryOrder(accountId: string) {
		this.settings.mailCategoryOrder = this.settings.mailCategoryOrder.filter((o) => o.accountId !== accountId);
		this.queueSave();
		this.notify();
	}
	...
	private i18nObserver: MutationObserver | null = null;
	...
	categoriesFor(accountId: string): OutlookCategory[] {
		// ordered here, at the one place every view reads them, so the tree, the
		// Categorize menu, the manage dialog and the filter list all agree
		return orderCategories(this.categoryCache.get(accountId) ?? [], this.categoryOrderFor(accountId));
	}
	...
	/** IMAP authorization codes are stored AES-256-GCM encrypted with a key
	 *  outside the vault; anything still plaintext from an older build is
	 *  migrated once, here at load. */
	private migrateImapSecrets() {
		let changed = false;
		for (const a of this.settings.imapAccounts) {
			if (a.password && !secretIsEncrypted(a.password)) {
				a.password = encryptSecret(a.password);
				changed = true;
			}
		}
		if (changed) this.queueSave();
	...
	async openMailView(): Promise<void> {
		if (this.settings.imapAccounts.length) {
			const existingImap = this.app.workspace.getLeavesOfType(VIEW_TYPE_MAIL_IMAP)[0];
			if (existingImap) {
				await this.app.workspace.revealLeaf(existingImap);
				return;
			}
			const imapLeaf = this.app.workspace.getLeaf(true);
			await imapLeaf.setViewState({ type: VIEW_TYPE_MAIL_IMAP, active: true });
			return;
		}
	...
	/** Make sure a file's parent folder exists: adapter.write refuses to
	 *  create directories itself, and a first write on a fresh vault dies
	 *  with ENOENT otherwise. */
...
class MailView extends ItemView {
	...
	/** The category being dragged to a new position in its account's list. */
	private categoryDrag: { accountId: string; name: string } | null = null;
	...
	/** The IMAP overlay panel and its working state. */
	private imapBtn: HTMLElement | null = null;
	private imapPane: HTMLElement | null = null;
	...
	async onOpen() {
		...
		measure();
		this.applyStartFolder();
		this.render();
	}

	/** Open where the settings say to, rather than always at All inboxes.
	 *
	 *  A folder that no longer exists, or belongs to an account that has gone,
	 *  falls back to the unified list rather than selecting nothing: an empty
	 *  pane with a name over it would look like a fetch that failed. The
	 *  Unread search folder and a category are both legitimate picks, and
	 *  neither is in the folder tree, so only real folder ids are checked
	 *  against it. */
	private applyStartFolder() {
		const st = this.plugin.settings.mailStart;
		const pick = st.mode === "last" ? this.plugin.settings.mailLastFolder : st.mode === "folder" ? { accountId: st.accountId, folderId: st.folderId, name: st.name } : null;
		if (!pick || !pick.folderId || !this.plugin.accountById(pick.accountId)) return;
		const virtual = pick.folderId === UNREAD_FOLDER || !!folderIdCategory(pick.folderId);
		if (!virtual && this.plugin.folderNamesFor(pick.accountId).size && !this.plugin.folderNamesFor(pick.accountId).has(pick.folderId)) return;
		this.folderSel = { ...pick };
		this.autoSelectPending = true;
		this.plugin.ensureFolderMail(pick.accountId, pick.folderId, false);
	}

	/** Keep the "where you left off" note current. Called from render, so it
	 *  follows every way of changing folder without each of them remembering
	 *  to say so. */
	private rememberFolder() {
		if (this.plugin.settings.mailStart.mode !== "last") return;
		const now = this.folderSel ? { accountId: this.folderSel.accountId, folderId: this.folderSel.folderId, name: this.folderSel.name } : null;
		const was = this.plugin.settings.mailLastFolder;
		const same = (!now && !was) || (!!now && !!was && now.accountId === was.accountId && now.folderId === was.folderId);
		if (same) return;
		this.plugin.settings.mailLastFolder = now;
		this.plugin.queueSave();
	}

	/** The "open here" entry every folder-ish row carries, so the pick can be
	 *  made where you already are rather than hunted for in settings. */
	private startHereItem(menu: Menu, sel: { accountId: string; folderId: string; name: string } | null) {
		const st = this.plugin.settings.mailStart;
		const isHere = sel ? st.mode === "folder" && st.accountId === sel.accountId && st.folderId === sel.folderId : st.mode === "all";
		menu.addItem((i) =>
			i
				.setTitle(isHere ? "Opens here when mail opens" : "Open here when mail opens")
				.setIcon(isHere ? "check" : "pin")
				.onClick(() => {
	...
	/** The IMAP panel: account, folder, message list, reading pane, and
	 *  reply. Self-contained so the Graph pane above never notices it. */
	private async renderImapPane(pane: HTMLElement) {
		...
		folderSel.empty();
		const refreshBtn = controls.createEl("button", { text: "Refresh", cls: "mod-cta" });
		const searchInput = controls.createEl("input", { type: "text", attr: { placeholder: "Search subjects" } });
		const drawFolders = async () => {
			...
			folderSel.empty();
			for (const f of folders) folderSel.createEl("option", { value: f, text: f });
			folder = folders.find((f) => /inbox/i.test(f)) ?? folders[0] ?? "";
		...
		refreshBtn.addEventListener("click", () => void drawList());
		searchInput.addEventListener("keydown", (e) => {
			if (e.key === "Enter") void drawList();
		});
		await drawFolders();
	}
	...
	private renderFolders(accounts: GraphAccount[], colorOf: Map<string, string>) {
		...
		host.empty();
		// All inboxes can be hidden: for someone who always works in one
		...
		if (!this.plugin.settings.mailHideAllInboxes) {
			...
			allRow.addEventListener("contextmenu", (e) => {
				e.preventDefault();
				const menu = new Menu();
				this.startHereItem(menu, null);
				menu.addSeparator();
				menu.addItem((i) =>
					i
						.setTitle("Hide All inboxes")
						.setIcon("eye-off")
						.onClick(() => {
		...
		if (favs.length) {
			...
			favs.forEach((fav, idx) => {
				...
				row.addEventListener("contextmenu", (e) => {
					...
					menu.addSeparator();
					this.startHereItem(menu, { accountId: acc.id, folderId: fav.folderId, name });
					menu.showAtMouseEvent(e);
				});
				row.draggable = true;
		...
		for (const a of accounts) {
			...
			for (const { folder, depth, expandable } of tree) {
				...
				row.addEventListener("contextmenu", (e) => {
					...
					this.startHereItem(menu, { accountId: a.id, folderId: folder.id, name: folder.name });
					menu.addItem((i) =>
			...
			const cats = this.plugin.categoriesFor(a.id);
			{
				...
				chead.addEventListener("contextmenu", (e) => {
					...
					// dragging a category onto another sets an order; this puts it
					// back, and stays disabled until there is an order to undo
					const arranged = this.plugin.categoryOrderFor(a.id).length > 0;
					menu.addSeparator();
					menu.addItem((i) => i.setTitle("Use the mailbox's own order again").setDisabled(!arranged).onClick(() => this.plugin.resetCategoryOrder(a.id)));
					menu.showAtMouseEvent(e);
				});
				if (!catCollapsed) {
					for (const cat of cats) {
						...
						// drag state is what keeps this apart from a message dropped
						// onto the row to tag it (acceptCategoryDrop, above)
						row.draggable = true;
						row.addEventListener("dragstart", (e) => {
							this.categoryDrag = { accountId: a.id, name: cat.displayName };
							e.stopPropagation();
							if (e.dataTransfer) e.dataTransfer.effectAllowed = "move";
						});
						row.addEventListener("dragend", () => {
						...
						row.addEventListener("drop", (e) => {
							...
							e.stopPropagation();
							this.categoryDrag = null;
							this.plugin.reorderCategory(a.id, d.name, cat.displayName);
						});
						row.addEventListener("click", () => {
							this.plugin.clearMailSearch();
						...
						row.addEventListener("contextmenu", (e) => {
							...
							this.startHereItem(menu, { accountId: a.id, folderId: fid, name: cat.displayName });
							menu.addItem((i) => i.setTitle("Manage categories...").setIcon("settings-2").onClick(() => new CategoriesModal(this.app, this.plugin, () => this.render()).open()));
							menu.showAtMouseEvent(e);
						});
					}
			...
			unreadRow.addEventListener("contextmenu", (e) => {
				...
				this.startHereItem(menu, { accountId: a.id, folderId: UNREAD_FOLDER, name: "Unread Mail" });
				menu.showAtMouseEvent(e);
			});
 ...
 */
function makeMovable(app: App, modal: Modal & { resetGeometry?: boolean }, key: string, fallback: { w: number; h: number }) {
	const stop = makeDraggable(app, modal.modalEl, modal.titleEl, key, fallback, undefined, modal.resetGeometry);
...
function makeDraggable(
	...
	const remember = () => {
		// reads of zero mean the element left the DOM (teardown timing), and
		// saving that would shrink the next open to the clamp minimums at 0,0
		if (el.offsetWidth < 50 || el.offsetHeight < 50) return;
		store.saveLocalStorage(key, { x: el.offsetLeft, y: el.offsetTop, w: el.offsetWidth, h: el.offsetHeight });
	};
...
/** Only the images Obsidian can render as a CSS background, sorted so a newly
 *  dropped wallpaper is the first thing the user sees. */
class VaultImagePickModal extends FuzzySuggestModal<TFile> {
	private static extensions = new Set(["jpg", "jpeg", "png", "gif", "webp", "avif", "svg"]);
	...
	constructor(
		...
		super(app);
		this.setPlaceholder(t("Choose a background image..."));
	}

	getItems(): TFile[] {
		return this.app.vault.getFiles().filter((f) => VaultImagePickModal.extensions.has(f.extension.toLowerCase())).sort((a, b) => b.stat.mtime - a.stat.mtime);
	}
...
function joinRecipients(values: string[]): string {
	const out: string[] = [];
	const seen = new Set<string>();
	for (const raw of values.join(",").split(",")) {
		const value = raw.trim();
		const key = /<([^>]+)>/.exec(value)?.[1]?.toLowerCase() ?? value.toLowerCase();
...
function quoteMail(body: ImapBody): string {
	const when = body.date ? new Date(body.date).toLocaleString() : "";
	const head = `<p><b>${escapeHtml(body.from || "sender")}</b> ${escapeHtml(when)}</p>`;
	const inner = body.html ? `<div>${body.html}</div>` : `<pre>${escapeHtml(body.text)}</pre>`;
	return `<div>${head}<blockquote style="margin:0 0 0 12px;padding-left:10px;border-left:2px solid currentColor">${inner}</blockquote></div>`;
}
...
/** IMAP-first mail view: a plain folder tree, a message list, and a reading
 *  pane. Kept separate from the Outlook view so the two cannot fight over
 *  the same layout. */
class ImapMailView extends ItemView {
	private foldersEl!: HTMLElement;
	private listEl!: HTMLElement;
	private readEl!: HTMLElement;
	private folderCache = new Map<string, ImapFolderInfo[]>();
	...
	async onOpen() {
		...
		const addHeaderAction = (key: string, icon: string, label: string, onClick: () => void) => {
			...
			this.actionBtns.set(key, b);
		};
		addHeaderAction("cacheAll", "database", "Cache all mail from every account and folder", () => void this.cacheAllMail());
		addHeaderAction("reply", "reply", "Reply", () => {
			const t = this.targetMessages();
			if (t.length === 1) this.compose("reply", t[0]);
		});
		addHeaderAction("replyAll", "reply-all", "Reply all", () => {
			const t = this.targetMessages();
			if (t.length === 1) this.compose("replyAll", t[0]);
		});
		addHeaderAction("forward", "forward", "Forward", () => {
			const t = this.targetMessages();
			if (t.length === 1) this.compose("forward", t[0]);
		});
		addHeaderAction("star", "star", "Star or unstar", () => void this.toggleHeaderFlag());
		addHeaderAction("read", "mail-open", "Mark read or unread", () => void this.toggleHeaderRead());
		addHeaderAction("move", "folder-input", "Move to folder", () => {
		...
		handle.addEventListener("click", () => {
			...
			root.toggleClass("is-list-collapsed", !folded);
			handle.empty();
	...
	private accounts(): ImapAccount[] {
		return this.plugin.settings.imapAccounts;
	}
	...
	private renderCachedTree() {
		this.folderCache.clear();
		for (const a of this.accounts()) this.folderCache.set(a.id, this.plugin.mailService.cachedFolderInfos(a.id));
		this.renderTree();
	}

	/** The network half of folder discovery. Cached names have already been
	 *  shown; this refreshes them and pulls the selected folder incrementally. */
	private async refreshFolders() {
		...
		await Promise.all(
			all.map(async (a) => {
				try {
				this.folderCache.set(a.id, await this.plugin.mailService.refreshFolderInfos(a));
				} catch (e) {
		...
		if (!this.accounts().some((a) => a.id === this.selectedAccountId)) this.selectedAccountId = all[0]?.id ?? "";
		const infos = this.folderCache.get(this.selectedAccountId) ?? [];
		const paths = infos.map((f) => f.path);
		if (!this.selectedFolder || !paths.includes(this.selectedFolder)) this.selectedFolder = paths.find((f) => /inbox/i.test(f)) ?? paths[0] ?? "";
	...
	private async selectAccount(id: string) {
	...
	private async selectFolder(id: string, folder: string) {
		this.selectedAccountId = id;
	...
	private currentAccount(): ImapAccount | null {
		return this.account(this.selectedAccountId);
	}

	private currentFolderInfos(): ImapFolderInfo[] {
		return this.folderCache.get(this.selectedAccountId) ?? [];
	}

	private isTrash(): boolean {
		const infos = this.currentFolderInfos();
		return !!this.selectedFolder && trashFolderFor(infos) === this.selectedFolder;
	}

	/** The left rail: one branch per connected account, each with its own
	 *  folders. Folder names are shown with the last path segment, and the
	...
	private renderTree() {
		...
		for (const a of accounts) {
			...
			head.addEventListener("contextmenu", (e) => {
				e.preventDefault();
				new Menu()
					.addItem((i) => i.setTitle("Reorder accounts").onClick(() => new ImapAccountOrderModal(this.app, this.plugin, () => this.refreshFolders()).open()))
					.showAtMouseEvent(e);
			});
			if (!open) continue;
			...
			const sorted = [...folders].sort((x, y) => {
				const weight = (p: string) => (/inbox/i.test(p) ? 0 : /drafts/i.test(p) ? 1 : /sent/i.test(p) ? 2 : /junk|spam/i.test(p) ? 3 : /trash|deleted/i.test(p) ? 4 : 10);
	...
	/** Fixed-height rows with visible-window rendering. The scroll box holds
	 *  one absolutely-positioned pool instead of spacers, so scrolling only
	 *  moves rows around and never rewrites scrollTop: wheel, trackpad, and
	 *  keyboard scrolling all stay native. */
	...
	private renderWindow() {
		const host = this.listEl;
		const inner = this.listInner;
		const a = this.currentAccount();
		if (!host || !inner || !a || !this.messages.length) return;
		const scrollTop = host.scrollTop;
		const viewport = host.clientHeight || 600;
		const start = Math.max(0, Math.floor(scrollTop / this.listRowHeight) - 5);
		const end = Math.min(this.messages.length, Math.ceil((scrollTop + viewport) / this.listRowHeight) + 5);
		const keep = new Set<number>();
		for (let i = start; i < end; i++) {
			const m = this.messages[i];
			if (!m) continue;
			keep.add(m.uid);
			let row = this.rowPool.get(m.uid);
			if (!row || !row.isConnected) {
				row = this.messageRow(inner, a, m);
				this.rowPool.set(m.uid, row);
			}
		...
		for (const [uid, row] of [...this.rowPool]) {
			if (!keep.has(uid)) {
				row.remove();
				this.rowPool.delete(uid);
			}
		}
	}

	private attachListScroll() {
		const host = this.listEl;
		...
		host.addEventListener("keydown", (e) => {
			...
			if (e.key === "Delete") {
				...
				if (!a) return;
				e.preventDefault();
				if (this.checked.size) void this.deleteChecked();
				else if (this.selectedMessage) void this.removeMessage(a, this.selectedMessage);
				return;
			}
			if (e.key === "ArrowDown") host.scrollTop += this.listRowHeight;
			else if (e.key === "ArrowUp") host.scrollTop -= this.listRowHeight;
			else if (e.key === "PageDown") host.scrollTop += host.clientHeight;
			else if (e.key === "PageUp") host.scrollTop -= host.clientHeight;
			else if (e.key === "Home") host.scrollTop = 0;
			else if (e.key === "End") host.scrollTop = host.scrollHeight;
			else return;
			e.preventDefault();
		});
	}

	/** Left/right in the list walks prev/next through the visible folder. */
	private stepMessage(dir: 1 | -1) {
		if (!this.messages.length) return;
		const open = this.selectedMessage;
		const i = open ? this.messages.findIndex((m) => m.uid === open.uid) : -1;
		if (i < 0 && dir < 0) return;
		const next = i < 0 ? 0 : Math.max(0, Math.min(this.messages.length - 1, i + dir));
		const m = this.messages[next];
		if (!m || m === this.selectedMessage) return;
		this.listEl?.focus();
		void this.openMessage(m);
		this.renderWindow();
		this.scrollRowIntoView(m.uid);
	}

	private scrollRowIntoView(uid: number) {
		const host = this.listEl;
		if (!host) return;
		const i = this.messages.findIndex((m) => m.uid === uid);
		if (i < 0) return;
		const top = i * this.listRowHeight;
		if (top < host.scrollTop) host.scrollTop = top;
		else if (top + this.listRowHeight > host.scrollTop + host.clientHeight) host.scrollTop = top + this.listRowHeight - host.clientHeight;
	}
	...
	private messageRow(host: HTMLElement, a: ImapAccount, m: ImapMessage): HTMLElement {
		...
		check.addEventListener("change", () => {
			if (check.checked) this.checked.add(m.uid);
		...
		row.addEventListener("contextmenu", (e) => {
			e.preventDefault();
			this.messageMenu(e, a, this.messages.find((x) => x.uid === m.uid) ?? m);
		});
		return row;
	}
	...
	private async deleteChecked() {
		const a = this.currentAccount();
		const uids = [...this.checked];
		if (!a || !uids.length) return;
		await this.deleteTargets(uids);
	}

	/** Remove a set of uids (trash-or-move rules included). More than one mail
	 *  at once asks for confirmation first; a single mail goes straight away. */
	private async deleteTargets(uids: number[]) {
		const a = this.currentAccount();
		if (!a || !uids.length) return;
		const run = async () => {
			try {
				if (this.isTrash()) await this.plugin.mailService.remove(a, this.selectedFolder, uids);
				else {
					const trash = trashFolderFor(this.currentFolderInfos());
					if (!trash) throw new Error("no trash folder found");
					await this.plugin.mailService.move(a, this.selectedFolder, trash, uids);
				}
				for (const id of uids) this.checked.delete(id);
				if (this.selectedMessage && uids.includes(this.selectedMessage.uid)) {
		...
		if (uids.length > 1) {
			new ConfirmModal(this.app, "Delete selected mail", `Move ${uids.length} messages to trash?`, "Delete", () => void run()).open();
		} else {
			await run();
		}
	}

	/** What the page-header actions act on: the checked mails when any boxes
	 *  are ticked, otherwise the mail open in the reading pane. */
	private targetMessages(): ImapMessage[] {
		if (!this.currentAccount()) return [];
		if (this.checked.size) {
			const map = new Map(this.messages.map((m) => [m.uid, m] as const));
			return [...this.checked].map((u) => map.get(u)).filter((m): m is ImapMessage => !!m);
		}
		return this.selectedMessage ? [this.selectedMessage] : [];
	}
	...
	private updateHeaderActions() {
		...
		const set = (key: string, enabled: boolean) => {
			const b = this.actionBtns.get(key);
			if (b) {
				b.disabled = !enabled;
				if (key === "star") b.toggleClass("is-active", one && targets[0].flagged);
			}
		};
		set("reply", one);
		set("replyAll", one);
		set("forward", one);
		set("star", targets.length > 0);
		set("read", targets.length > 0);
		set("move", targets.length > 0 && this.currentFolderInfos().length > 1);
		set("delete", targets.length > 0);
	}

	private async toggleHeaderFlag() {
		const a = this.currentAccount();
		const targets = this.targetMessages();
		if (!a || !targets.length) return;
		const makeFlagged = !targets.every((m) => m.flagged);
		try {
	...
	private async toggleHeaderRead() {
		const a = this.currentAccount();
		const targets = this.targetMessages();
		if (!a || !targets.length) return;
		const read = targets.some((m) => m.unread);
		try {
	...
	private pickFolderForTargets(a: ImapAccount) {
		const targets = this.targetMessages();
		if (!targets.length) return;
	...
	private messageMenu(e: MouseEvent, a: ImapAccount, m: ImapMessage) {
		...
		new Menu()
			...
			.addItem((i) =>
				...
				.setDisabled(!m.subject && !m.from)
				.onClick(() =>
					new EventModal(this.app, this.plugin, null, new Date(m.date).getTime() || Date.now(), (new Date(m.date).getTime() || Date.now()) + 30 * 60000, false, {
						title: m.subject,
						invites: m.from,
					}).open()
				)
	...
	private async removeMessage(a: ImapAccount, m: ImapMessage) {
		try {
			if (this.isTrash()) await this.plugin.mailService.remove(a, this.selectedFolder, [m.uid]);
			else {
				const trash = trashFolderFor(this.currentFolderInfos());
				if (!trash) throw new Error("no trash folder found");
				await this.plugin.mailService.move(a, this.selectedFolder, trash, [m.uid]);
			}
			if (this.selectedMessage?.uid === m.uid) {
				await this.clearReadPaneKeepingNeighbor(m);
				return;
			}
			await this.loadMessages();
		} catch (e) {
	...
	/** The mail to show once the open one is gone: the next one, else the
	 *  previous, so deleting from the reading pane never strands you blank. */
	private neighborOf(m: ImapMessage): ImapMessage | null {
		const i = this.messages.findIndex((x) => x.uid === m.uid);
		if (i < 0) return null;
		return this.messages[i + 1] ?? this.messages[i - 1] ?? null;
	}

	private async clearReadPaneKeepingNeighbor(m: ImapMessage) {
		const neighbor = this.neighborOf(m);
		this.selectedMessage = null;
		this.selectedBody = null;
		this.readEl.empty();
		await this.loadMessages();
		if (neighbor) {
			const cur = this.messages.find((x) => x.uid === neighbor.uid) ?? null;
			if (cur) {
				this.listEl?.focus();
				void this.openMessage(cur);
				this.renderWindow();
				this.scrollRowIntoView(cur.uid);
			}
		}
	}

	private pickFolder(a: ImapAccount, m: ImapMessage) {
		const infos = this.folderCache.get(a.id) ?? this.plugin.mailService.cachedFolderInfos(a.id);
		new ImapFolderPickModal(this.app, infos.filter((f) => f.path !== this.selectedFolder), (folder) => void this.moveMessage(a, m, folder)).open();
	}

	private async moveMessage(a: ImapAccount, m: ImapMessage, destination: string) {
		const source = this.selectedFolder;
		try {
			await this.plugin.mailService.move(a, source, destination, [m.uid]);
			if (this.selectedMessage?.uid === m.uid) {
				this.selectedMessage = null;
				this.selectedBody = null;
				this.readEl.empty();
	...
	private openDraft(d: ImapDraft) {
		const a = this.account(d.accountId);
		if (!a) return;
		new ImapRichComposeModal(this.app, this.plugin, a, d.id, {
			to: d.to,
			cc: d.cc,
			bcc: d.bcc,
			subject: d.subject,
	 ...
	 *  every body pulled to disk, so reading afterwards is instant and works
	...
	private async messageBody(a: ImapAccount, folder: string, m: ImapMessage): Promise<ImapBody> {
		const key = `${a.id}:${folder}:${m.uid}`;
		const cached = this.bodyCache.get(key);
		if (cached) return cached;
		const body = await this.plugin.mailService.body(a, folder, m.uid);
	...
	private renderMessage() {
		...
		if (body.html) {
			...
			bodyEl.appendChild(sanitizeHTMLToDom(body.html));
		} else {
	...
	private compose(kind: "new" | "reply" | "replyAll" | "forward" = "new", message?: ImapMessage) {
		const a = this.currentAccount();
		if (!a) return;
	...
	private async loadBodyFor(
		kind: "new" | "reply" | "replyAll" | "forward",
		a: ImapAccount,
		m?: ImapMessage
	): Promise<ImapComposeSeed> {
		if (!m || kind === "new") return { to: "", cc: "", bcc: "", subject: "", html: "", attachments: [] };
		const body = await this.messageBody(a, this.selectedFolder, m);
		const subject = kind === "forward" ? `Fwd: ${m.subject}` : `Re: ${m.subject}`;
		const to = kind === "forward" ? "" : kind === "replyAll" ? joinRecipients([body.to, m.replyTo || m.from]) : (m.replyTo || m.from);
...
/** A compact folder chooser for move operations. It deliberately takes a
 *  preloaded list instead of opening a network call inside the menu. */
class ImapFolderPickModal extends Modal {
	constructor(
		app: App,
		private folders: ImapFolderInfo[],
...
/** A tiny order modal for IMAP accounts. Up and down buttons are enough: the
 *  list is short and the change is immediately saved to settings. */
class ImapAccountOrderModal extends Modal {
	...
	onOpen() {
		this.titleEl.setText("Adjust IMAP account order");
		const c = this.contentEl;
		this.renderRows(c);
	}
...
class PowerCalendarView extends ItemView {
	...
	private applyBackground() {
		...
		const f = path ? this.app.vault.getAbstractFileByPath(normalizePath(path)) : null;
		el.style.backgroundImage = f instanceof TFile ? `url("${this.app.vault.getResourcePath(f)}")` : "";
		el.style.opacity = String(Math.max(0, Math.min(100, s.calBackgroundOpacity)) / 100);
	}
	...
	private renderSidebar() {
		...
		const paint = () => {
			...
			for (const cell of cells) {
				...
				d.toggleClass("has-events", eventsOnDay(events, cell.key).length > 0);
				d.addEventListener("click", () => {
					this.anchorKey = cell.key;
					this.refresh(false);
				});
	...
	private sidebarGroups(): { label: string; items: SidebarCal[] }[] {
		...
		if (s.localCalendars.length)
			groups.push({ label: "Local calendars", items: s.localCalendars.map((lc) => ({ name: lc.name, color: lc.color, enabled: lc.enabled, setEnabled: (v: boolean) => (lc.enabled = v), setColor: (v: string) => (lc.color = v) })) });
	...
	private render() {
		...
		// the tasks mode owns the body entirely: no event grid, no fetch window
		const events = this.mode === "tasks" ? [] : this.applyFilter(this.plugin.eventsForWindow(win.fromKey, win.toKey));

		this.bodyEl.empty();
		if (this.mode === "month") this.renderMonth(events);
		else if (this.mode === "agenda") this.renderAgenda(events, win.fromKey, win.toKey);
		else if (this.mode === "tasks") this.renderTasks();
	...
	private attachQuickDelete(el: HTMLElement, ev: PCEvent) {
		...
		el.addEventListener("keydown", (e) => {
			...
			if (!ev.canEdit || ev.notePath) return;
			e.preventDefault();
			e.stopPropagation();
	...
	/** The Tasks mode: a long-term kanban living where the event grid would
	 *  be. The board list and its data come from Markdown files in the
	 *  boards folder; this adapter wires them to the vault. */
	private renderTasks() {
		const folder = this.plugin.settings.kanbanFolder.trim() || "Boards";
		const adapter = this.plugin.app.vault.adapter;
		const app = this.plugin.app;
		const host: KanbanHost = {
			readBoard: async (path) => adapter.read(normalizePath(path)),
			writeBoard: async (path, text) => {
				// the folder, not the file: a path with the .md name would
				// create a directory that shadows the file
				const p = normalizePath(path);
				const dir = p.includes("/") ? p.slice(0, p.lastIndexOf("/")) : "";
				if (dir) await this.plugin.ensureFolder(dir);
				await adapter.write(normalizePath(path), text);
			},
			listBoards: async (dir) => {
				try {
					const listing = await adapter.list(normalizePath(dir));
					// full vault paths, so reads never resolve to the vault root
					return listing.files.filter((f) => /\.md$/i.test(f)).sort((a, b) => a.localeCompare(b));
				} catch {
					return [];
				}
			},
			...
			renameBoard: async (path, name) => {
				...
				const to = normalizePath(`${from.split("/").slice(0, -1).join("/")}/${name.trim().replace(/\.md$/i, "")}.md`);
				await adapter.rename(from, to);
				return to;
			},
			deleteBoard: async (path) => {
			...
			linkToCalendar: (text) => {
				...
				const day = date ?? keyOfMs(Date.now());
				const start = msOfKey(day) + 9 * 3600000;
				this.openEventModal(null, start, start + 60 * 60000, false, { title: text.replace(/📅\s*\d{4}-\d{2}-\d{2}/g, "").trim() });
			},
		};
		const board = new TasksBoard(this.app, host, this.bodyEl!, folder);
		void board.render();
	}
	...
	private async addEventToTasks(ev: PCEvent) {
		const folder = this.plugin.settings.kanbanFolder.trim() || "Boards";
		const adapter = this.plugin.app.vault.adapter;
		let files: string[] = [];
		try {
			const listing = await adapter.list(normalizePath(folder));
			files = listing.files.filter((f) => /\.md$/i.test(f)).sort((a, b) => a.localeCompare(b));
		} catch {
		...
		try {
			...
			const dir = p.includes("/") ? p.slice(0, p.lastIndexOf("/")) : "";
			if (dir) await this.plugin.ensureFolder(dir);
			const board = parseKanbanBoard(await adapter.read(p));
			const col = board.columns.find((c) => /^(todo|to-do|待办)$/i.test(c.name.trim())) ?? board.columns[0];
			if (!col) return;
			const span = eventDaySpan(ev);
			const body: string[] = [];
			if (span.startKey !== span.endKey) body.push(`日期：${span.startKey} - ${span.endKey}`);
			if (!ev.allDay) body.push(`时间：${fmtEventRange(ev, this.plugin.settings.use24h)}`);
			const loc = ev.location?.trim() ?? "";
			if (loc) body.push(`地点：${loc}`);
			const desc = stripMeetingBoilerplate(ev.description ?? "").trim();
			if (desc) body.push(`说明：${desc}`);
			if (ev.calendarName) body.push(`日历：${ev.calendarName}`);
			if (ev.joinUrl) body.push(`会议链接：${ev.joinUrl}`);
			if (ev.url && ev.url !== ev.joinUrl) body.push(`来源：${ev.url}`);
			const title = ev.title === "(no title)" ? "未命名日程" : ev.title.trim();
			col.cards.push({
				text: `${title} 📅 ${span.startKey}`,
				checked: false,
	...
	private openCard(ev: PCEvent, anchor: HTMLElement) {
		...
		const task = btns.createEl("button", { text: "加到任务" });
...
class EventModal extends Modal {
	...
	/** Every open lands centered at the fallback size: a remembered geometry
	 *  gone stale once cost a whole session of tiny, clipped opens. */
	resetGeometry = true;
		...
		if (!this.ev && targets.length > 1) {
			...
			new Setting(titleRow).setName(t("Calendar")).addDropdown((d) => {
				...
				d.setValue(this.targetKey ?? targets[0].key).onChange((v) => {
					this.targetKey = v;
					// the meeting toggle names the provider it will land on, so it
					// follows the calendar from Teams to Meet and back
	...
	/** The online-meeting toggle, whose name follows the calendar it will land
	 *  on: a Teams meeting on Microsoft 365, a Google Meet on Google. Redrawn
	 *  when the calendar changes. New events only — a provider mints the link
	 *  once with the event, and adding or pulling one on an existing meeting is
	 *  a different, riskier write than this offers. Teams is a work/school
	 *  feature, so a personal Microsoft account is not offered a toggle that
	 *  would only fail; a source that hosts neither shows nothing at all. */
	private renderMeetingField() {
		const host = this.meetingEl;
		if (!host) return;
		host.empty();
		if (this.ev) return;
		const targets = this.plugin.writableTargets();
		const def = ((this.targetKey ? targets.find((t) => t.key === this.targetKey) : null) ?? this.plugin.defaultWriteTarget())?.def;
		let kind: "teams" | "meet" | null = null;
		if (def?.kind === "m365" && !this.plugin.isPersonalAccount(def.account)) kind = "teams";
		else if (def?.kind === "google") kind = "meet";
		if (!kind) {
			this.online = false;
			return;
		}
		const label = kind === "teams" ? "Teams meeting" : "Google Meet";
		const desc = kind === "teams" ? "Host it on Microsoft Teams. The join link is created with the event." : "Add Google Meet video conferencing. The join link is created with the event.";
		new Setting(host).setName(label).setDesc(desc).addToggle((t) => t.setValue(this.online).onChange((v) => (this.online = v)));
	}
	...
	private async save() {
		...
		const draft: EventDraft = {
			...
			showAs: this.showAs,
			onlineMeeting: this.ev ? undefined : this.online || undefined,
		};
...
class LocalCalendarModal extends Modal {
	private name: string;
	private path: string;

	constructor(
		app: App,
		private existing: LocalCalendarSource | null,
		private onSave: (src: LocalCalendarSource) => void
	) {
		super(app);
		this.name = existing?.name ?? "";
		this.path = existing?.path ?? "Calendar/Local.ics";
	}
	...
	onOpen() {
		...
		btns.createEl("button", { text: "Save", cls: "mod-cta" }).addEventListener("click", () => {
			...
			this.onSave({
				id: this.existing?.id ?? freshId(),
				name: this.name.trim() || "Local calendar",
				path: this.path.trim(),
				color: this.existing?.color ?? "#e8a33d",
				enabled: this.existing?.enabled ?? true,
			});
			this.close();
		});
...
class ImapAccountModal extends Modal {
	constructor(
		app: App,
		private existing: ImapAccount | null,
		private onSave: (a: ImapAccount) => void
	) {
		super(app);
	}
	...
	onOpen() {
		...
		const label = e?.label ?? "";
		const user = e?.user ?? "";
		const password = decryptSecret(e?.password ?? "");
		const imapHost = e?.imapHost ?? "";
		const imapPort = String(e?.imapPort ?? 993);
	...
	private persist(a: ImapAccount, btns: HTMLElement) {
		void btns;
		this.onSave(a);
	}
...
/** Saved IMAP drafts: ordinary text and attachments, kept in data.json. */
class ImapDraftListModal extends Modal {
	...
	onOpen() {
		this.titleEl.setText("Saved drafts");
		const c = this.contentEl;
		...
		for (const d of [...drafts].sort((x, y) => y.updatedMs - x.updatedMs)) {
			...
			btns.createEl("button", { text: "Delete" }).addEventListener("click", () => {
...
/** A desktop-style IMAP compose window: rich text, CC/BCC, attachments and
 *  local drafts. It does not share the Outlook editor because that class is
 *  bound to Graph accounts; here only the reusable editor helpers are used. */
class ImapRichComposeModal extends Modal {
	private to = "";
	private cc = "";
	private bcc = "";
	private subject = "";
	...
	constructor(
		...
		private onSend?: (mail: SmtpMail) => void
	) {
		super(app);
		this.to = seed.to;
		this.cc = seed.cc;
	...
	private mail(): SmtpMail {
		return {
			to: this.to,
			cc: this.cc,
			bcc: this.bcc,
			subject: this.subject,
			text: this.editorEl.innerText.trim(),
			html: this.editorEl.innerHTML,
			inReplyTo: this.seed.inReplyTo,
			attachments: this.attachments,
		};
	}

	private hasContent(): boolean {
		return !!(this.to.trim() || this.cc.trim() || this.bcc.trim() || this.subject.trim() || this.attachments.length || (this.editorEl.innerText.trim() && this.editorEl.innerText !== "\u200b"));
	}

	private async saveDraft(): Promise<void> {
		if (!this.hasContent()) return;
		const mail = this.mail();
		const draft: ImapDraft = {
			id: this.draftId ?? `${Date.now()}-${Math.random().toString(16).slice(2)}`,
			...
			html: mail.html ?? "",
			attachments: this.attachments.map((a) => ({ filename: a.filename, contentType: a.contentType ?? "application/octet-stream", base64: a.content.toString("base64") })),
			updatedMs: Date.now(),
		};
		const rest = this.plugin.settings.imapDrafts.filter((x) => x.id !== draft.id);
	...
	onClose() {
		// A window closed by accident keeps the work; an explicit save gives
		// the user a named point to return to, and Send removes the draft.
		if (!this.sent && this.hasContent()) void this.saveDraft();
		this.contentEl.empty();
	}
}
...
class NyaHomeSettingTab extends PluginSettingTab {
	...
	private localHost: HTMLElement | null = null;
	private imapHost: HTMLElement | null = null;
	...
	private drawGraphAccounts() {
		...
		for (const a of s.graphAccounts) {
			...
			// sender photos come out of a work directory, so a personal account
			// is not missing them, it simply has nowhere to get them from
			if (a.refresh && this.plugin.canMailAccount(a) && !this.plugin.isPersonalAccount(a) && !a.grantedScope.includes("ProfilePhoto")) missing.push("sender photos");
	...
	/** The local .ics calendars. */
	private drawLocalCalendars() {
		const host = this.localHost;
		if (!host) return;
		host.empty();
		const s = this.plugin.settings;
		const save = () => this.plugin.queueSave();
		for (const lc of s.localCalendars) {
			new Setting(host)
				.setName(lc.name)
				.setDesc(lc.path)
				.addColorPicker((p) =>
					p.setValue(lc.color || "#e8a33d").onChange((v) => {
						lc.color = v;
						save();
						this.plugin.sourcesChanged();
					})
				...
				.addToggle((t) =>
					t.setValue(lc.enabled).onChange((v) => {
						lc.enabled = v;
						save();
						this.plugin.sourcesChanged();
					})
				...
				.addButton((b) =>
					b.setButtonText("Edit").onClick(() => {
						new LocalCalendarModal(this.app, lc, (updated) => {
							const i = s.localCalendars.findIndex((x) => x.id === updated.id);
							if (i >= 0) s.localCalendars[i] = updated;
							s.localCalendars = [...s.localCalendars];
							save();
							this.plugin.sourcesChanged();
				...
				.addButton((b) =>
					markDestructive(b.setButtonText("Remove")).onClick(() => {
						s.localCalendars = s.localCalendars.filter((x) => x.id !== lc.id);
						save();
						this.plugin.sourcesChanged();
						this.drawLocalCalendars();
					})
	...
	/** The IMAP mailboxes. */
	private drawImapAccounts() {
		const host = this.imapHost;
		if (!host) return;
		host.empty();
		const s = this.plugin.settings;
		const save = () => this.plugin.queueSave();
		for (const a of s.imapAccounts) {
			new Setting(host)
				.setName(imapLabel(a))
				.setDesc(a.user)
				.addButton((b) =>
					b.setButtonText("Edit").onClick(() => {
						new ImapAccountModal(this.app, a, (updated) => {
							const i = s.imapAccounts.findIndex((x) => x.id === updated.id);
							if (i >= 0) s.imapAccounts[i] = updated;
							s.imapAccounts = [...s.imapAccounts];
							save();
				...
				.addButton((b) =>
					markDestructive(b.setButtonText("Remove")).onClick(() => {
						s.imapAccounts = s.imapAccounts.filter((x) => x.id !== a.id);
						save();
						this.drawImapAccounts();
					})
				);
	...
	private buildPages(): Page[] {
		...
		const local: Row[] = [
			intro("Calendars stored as .ics files in this vault. Create, edit, and drag events freely — everything is written to the file, which any calendar app can read and which syncs wherever your vault goes."),
			this.listRow(
				"Local calendars",
				["ics file", "offline", "file-backed"],
			...
			),
			{
				...
				build: (st) => {
					st.addButton((b) =>
						b.setButtonText("Add calendar").onClick(() => {
							new LocalCalendarModal(this.app, null, (src) => {
								s.localCalendars = [...s.localCalendars, src];
								save();
								this.plugin.sourcesChanged();
								this.drawLocalCalendars();
							}).open();
		...
		const imap: Row[] = [
			...
			),
			{
				...
				build: (st) => {
					st.addButton((b) =>
						b.setButtonText("Add account").onClick(() => {
							new ImapAccountModal(this.app, null, (a) => {
								s.imapAccounts = [...s.imapAccounts.filter((x) => x.id !== a.id), a];
								save();
							}).open();
						})
					);
		...
		const views: Row[] = [
			{
				...
				help: "Pick any image stored in the vault; Obsidian streams it from the vault itself, so no copy is made outside the library. A missing or renamed file is treated as no image rather than an error.",
				build: (st) => {
					st.addText((t) =>
						t.setPlaceholder("Attachments/wallpaper.jpg").setValue(s.calBackgroundPath).onChange((v) => {
							s.calBackgroundPath = v;
							save();
							this.plugin.notify();
						})
					);
			...
			},
			{
				...
				desc: `${s.calBackgroundOpacity}%`,
				help: "How strongly the wallpaper shows through. 0% turns the image off entirely; 100% shows it at full strength.",
				build: (st) => {
					st.addSlider((sl) =>
						showSliderValue(sl)
							.setLimits(0, 100, 1)
							.setValue(s.calBackgroundOpacity)
							.onChange((v) => {
								s.calBackgroundOpacity = v;
								st.setDesc(`${v}%`);
								save();
								this.plugin.notify();
							})
			...
			},
			{
				...
				build: (st) => {
					st.addDropdown((d) =>
						d
							.addOptions({ month: "Month", week: "Week", workweek: "Work week", day: "Day", agenda: "Agenda", tasks: "Tasks" })
		...
		const mail: Row[] = [
			{
				...
				desc: "Which list the mail view shows when you open it.",
				aliases: ["default folder", "start", "startup", "all inboxes", "home"],
				help: "All inboxes is the unified list across every mailbox. Where you left off reopens whatever folder you were last looking at. A folder you picked is set by right-clicking any folder, favorite, or category and choosing \"Open here when mail opens\", which is also how you change it: the option only appears in this list once you have picked one, because there is nothing to name until then.",
				build: (st) => {
					const cur = s.mailStart;
					st.addDropdown((d) => {
						d.addOption("all", "All inboxes");
						d.addOption("last", "Where you left off");
						if (cur.mode === "folder" && cur.name) d.addOption("folder", cur.name);
						d.setValue(cur.mode).onChange((v) => {
							// switching away from a pick keeps its fields, so
							// switching back does not ask for the folder again
							s.mailStart = { ...cur, mode: v as "all" | "last" | "folder" };
							save();
						});
					});
				},
			...
			{
				name: "Show All inboxes",
				desc: "The unified row at the top of the folder pane. Hiding it leaves the command palette as the way there.",
				aliases: ["unified", "hide", "remove all inboxes"],
				build: (st) => {
					st.addToggle((t) =>
						t.setValue(!s.mailHideAllInboxes).onChange((v) => {
							s.mailHideAllInboxes = !v;
							save();
							this.plugin.notify();
						})
					);
		...
		mail.push(
			...
			},
			{
				...
				help: "Pictures come from your organization's directory, so colleagues have one and outside senders almost never do; anyone without falls back to the lettered circle. It needs the delegated ProfilePhoto.Read.All permission on the Azure app registration and a Reconnect on each work account, and until then this quietly does nothing rather than erroring. A personal account has no directory to read, so it never contributes faces and is never asked for the permission. Faces are fetched a few at a time as a list is drawn, cached, and kept in the disposable cache file so they are there the next time you open Obsidian.",
				build: (st) => {
					st.addToggle((t) =>
		...
		for (const p of pages) {
			...
			for (const g of p.groups) {
				if (g.heading) g.heading = t(g.heading);
				for (const r of g.rows) {
					if (r.name) r.name = t(r.name);
					if (r.desc) r.desc = t(r.desc);
					if (r.help) r.help = t(r.help);
				}
			}
		}
		return pages;
