...
// --- categories ---
{
	...
	// orderCategories: a dragged name leads, the rest keep the mailbox's order
	const cats = [{ displayName: "Alpha" }, { displayName: "Bravo" }, { displayName: "Charlie" }, { displayName: "Delta" }];
	const names = (cs: { displayName: string }[]) => cs.map((c) => c.displayName);
	eq(names(orderCategories(cats)), ["Alpha", "Bravo", "Charlie", "Delta"], "no custom order leaves the mailbox's order untouched");
	eq(names(orderCategories(cats, [])), ["Alpha", "Bravo", "Charlie", "Delta"], "an empty order is the same as none");
	eq(names(orderCategories(cats, ["Charlie", "Alpha"])), ["Charlie", "Alpha", "Bravo", "Delta"], "named ones lead in order, the rest keep the mailbox's order");
	eq(names(orderCategories(cats, ["charlie"])), ["Charlie", "Alpha", "Bravo", "Delta"], "matching a name ignores case");
	eq(names(orderCategories(cats, ["Zulu"])), ["Alpha", "Bravo", "Charlie", "Delta"], "a name no longer in the mailbox places nothing");
	eq(names(orderCategories(cats, ["Delta", "Bravo", "Charlie", "Alpha"])), ["Delta", "Bravo", "Charlie", "Alpha"], "a full arrangement is honored end to end");
	eq(names(cats), ["Alpha", "Bravo", "Charlie", "Delta"], "ordering does not mutate the input list");
...
eq(googleRecurrence("weekdays", "2026-07-17"), ["RRULE:FREQ=WEEKLY;BYDAY=MO,TU,WE,TH,FR"], "google weekdays rrule");
{
	...
	// online meeting: a Teams meeting on Graph, a Meet on Google
	const teams = graphEventBody({ title: "Sync", startMs, endMs: startMs + 30 * 60000, allDay: false, onlineMeeting: true }, "UTC");
	eq(teams.isOnlineMeeting, true, "graph online meeting sets isOnlineMeeting");
	eq(teams.onlineMeetingProvider, "teamsForBusiness", "graph online meeting asks for Teams");
	eq("isOnlineMeeting" in noTouch, false, "no online meeting, nothing sent to graph");
	eq("onlineMeetingProvider" in noTouch, false, "no provider sent to graph either");
	const meet = googleEventBody({ title: "Sync", startMs, endMs: startMs + 30 * 60000, allDay: false, onlineMeeting: true }, "UTC");
	const conf = meet.conferenceData as { createRequest: { requestId: string; conferenceSolutionKey: { type: string } } };
	eq(conf.createRequest.conferenceSolutionKey.type, "hangoutsMeet", "google online meeting asks for Meet");
	eq(typeof conf.createRequest.requestId === "string" && conf.createRequest.requestId.length > 0, true, "google Meet carries a request id");
	eq("conferenceData" in googleEventBody({ title: "Sync", startMs, endMs: startMs + 30 * 60000, allDay: false }, "UTC"), false, "no online meeting, no conferenceData on google");
}
{
	const raw = {
		id: "R",
...

{
	...
	const before = parseIcsEvents(text, msOfKey("2026-07-14"), msOfKey("2026-07-18"), SRC);
	const moved = before.find((e) => e.title === "Daily (moved)");
	eq(moved?.recurrenceIdMs, Date.UTC(2026, 6, 15, 10, 0, 0), "ics: a moved occurrence keeps its original recurrence id");
	eq(moved?.recurrenceIdAllDay, false, "ics: a moved occurrence knows its original recurrence type");
	text = upsertIcsOverride(
		text,
		"override-edit",
		moved?.recurrenceIdMs ?? 0,
		{
			uid: "override-edit",
			title: "Daily (moved again)",
			startMs: Date.UTC(2026, 6, 15, 15, 0, 0),
			endMs: Date.UTC(2026, 6, 15, 15, 30, 0),
			allDay: false,
		},
		moved?.recurrenceIdAllDay ?? false
	);
	const after = parseIcsEvents(text, msOfKey("2026-07-14"), msOfKey("2026-07-18"), SRC);
	eq(after.some((e) => e.startMs === Date.UTC(2026, 6, 15, 14, 0, 0)), false, "ics: editing a moved occurrence updates the old override");
	eq(after.some((e) => e.title === "Daily (moved again)" && e.startMs === Date.UTC(2026, 6, 15, 15, 0, 0)), true, "ics: editing a moved occurrence lands at its new time");
	eq(after.filter((e) => e.title.includes("moved")).length, 1, "ics: editing a moved occurrence does not duplicate it");
}
...
// --- which Microsoft accounts are personal (it decides what to ask consent for) ---
{
	eq(isPersonalMicrosoft(true, "fd64b9b8-7d31-494b-aedc-5c64cd1bd339", "someone@contoso.com"), true, "a recorded verdict beats the tenant");
	eq(isPersonalMicrosoft(false, "consumers", "someone@outlook.com"), false, "a recorded verdict beats the address too");
	eq(isPersonalMicrosoft(undefined, "consumers", "someone@contoso.com"), true, "'consumers' admits personal accounts only");
	eq(isPersonalMicrosoft(undefined, "CONSUMERS", "someone@contoso.com"), true, "the authority is matched case-insensitively");
	// the account that could not reconnect: pinned to 'common', personal all the same
	eq(isPersonalMicrosoft(undefined, "common", "steve@outlook.com"), true, "an outlook.com address at 'common' is personal");
	eq(isPersonalMicrosoft(undefined, "common", "Steve@Hotmail.COM"), true, "the address is matched case-insensitively");
	eq(isPersonalMicrosoft(undefined, "common", "someone@live.com"), true, "live.com is personal");
	eq(isPersonalMicrosoft(undefined, "common", "someone@msn.com"), true, "msn.com is personal");
	eq(isPersonalMicrosoft(undefined, "common", "someone@contoso.com"), false, "a work address at 'common' is not personal");
	eq(isPersonalMicrosoft(undefined, "fd64b9b8-7d31-494b-aedc-5c64cd1bd339", "steve@irely.com"), false, "a single-tenant work account is not personal");
	eq(isPersonalMicrosoft(undefined, "common", "Microsoft 365"), false, "a row with no address yet is not personal");
	eq(isPersonalMicrosoft(undefined, "common", ""), false, "an empty address is not personal");
	eq(MSA_TENANT, "9188040d-6c67-4c5b-b112-36a304b66dad", "the tid claim personal accounts arrive with");
}
